...
- Assumption is each user should belong to particular store. So user can view and edit information for that store.
- It is fine for user to wait for for some second (for large feed) status of the file upload. As for large file it could take time for upload.
...
- we will not proceed if some records fails during loading the feed file.
- Not considering Notification feature during feed file failure to notify user with detailed report.
##### 6.Api Design
   ![Upload service](diagrams/api-file.png)
   ![Upload service](diagrams/api-product-pm.png)
...
- more security and validations
- Handle different scenario during file upload , mostly in case of failure to load fed file.

# 🚀 Project Setup & Usage
## demo
[here](https://drive.google.com/file/d/11V6zQn75cRDA6hnW8a8HISYwEW1bcfoI/view?usp=sharing).
